...
import com.ikev2.Ikev2Module;
import com.ikev2.NotificationBundle;
import com.ikev2.R;
  ...
  private Bundle mNotificationBundle;
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
          ...
          //AUDIT
          mNotificationBundle = bundle.getBundle(NotificationBundle.BUNDLE_KEY);
				}
			}
    ...
    String s2 = "";
        ...
        //AUDIT
        /**
         * Disable Notification Action for Error Note, [TODO]
         */
...
//
		...
		else
		{
			...
			{
				case CONNECTING:
          s2 = mNotificationBundle.getString(NotificationBundle.TITLE_CONNECTING);
					...
					break;
				case CONNECTED:
          s2 = mNotificationBundle.getString(NotificationBundle.TITLE_CONNECTED);
          s = R.string.state_connected;
					builder.setColor(ContextCompat.getColor(this, R.color.success_text));
					if(mNotificationBundle.getBoolean(NotificationBundle.SHOW_TIMER)){
            builder.setUsesChronometer(true);
          }
					add_action = true;
					break;
				case DISCONNECTING:
          s2 = mNotificationBundle.getString(NotificationBundle.TITLE_DISCONNECTING);
    ...
    if(s2!=null && !s2.isEmpty()){
      ...
      //AUDIT
      /**
       * Disable Disconnect ACTION for connected State [TODO]
       */
			if (add_action && mNotificationBundle.getBoolean(NotificationBundle.SHOW_DISCONNECT))
			{
				Intent intent = new Intent(getApplicationContext(), VpnProfileControlActivity.class);
				...
				PendingIntent pending = PendingIntent.getActivity(getApplicationContext(), 0, intent,
																  flags);
        String actionTitle = "";
        if(mNotificationBundle.getString(NotificationBundle.TITLE_ACTION_DISCONNECT)==null){
          actionTitle = getString(R.string.disconnect);
        }else{
          actionTitle = mNotificationBundle.getString(NotificationBundle.TITLE_ACTION_DISCONNECT);
        }
				builder.addAction(R.drawable.ic_notification_disconnected,actionTitle, pending);
			}
			if (error == ErrorState.NO_ERROR)
			{
    ...
    /**
     * Disable Open App Action on TAP. [TODO]
     */


    try{
      if(mNotificationBundle.getString(NotificationBundle.ACTIVITY_PACKAGE_NAME)!=null){
        Intent intent = new Intent(getApplicationContext(),Class.forName(Objects.requireNonNull(mNotificationBundle.getString(NotificationBundle.ACTIVITY_PACKAGE_NAME))));
        PendingIntent pending = PendingIntent.getActivity(getApplicationContext(), 0, intent,
          flags);
